...
callButton.onclick = () => {
  ...
  const wsUrl = `${protocol}//${window.location.host}/ws/meetings/${roomName}/`;
  ...
  function createPeerConnection(peerId, ws) {
    const servers = null; // Define your STUN/TURN servers configuration if needed
...
// callButton.onclick = () => {
//   const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
//   const ws = new WebSocket(protocol + '//' + window.location.host + '/ws/meetings/' + roomName + '/');

//   // Define peerConnections and localStream globally if not already done
//   let peerConnections = {};
//   let localStream;
//   let screenStream;
//   const remoteVideos = document.getElementById('remoteVideos');
...
//   // const ws = new WebSocket('ws://' + window.location.host + '/ws/meetings/' + roomName + '/');
